#!/usr/bin/env python3
"""Figure 4 variant: model-observation scatter density plots with statistics."""
...
DEFAULT_OUTPUT = SCRIPT_DIR / "F4-1.pdf"
...
def expanded_limit(limit: float) -> float:
    if limit <= 60:
        step = 10.0
    elif limit <= 180:
        step = 25.0
        ...
        step = 500.0
    return max(step, np.ceil(limit * 1.16 / step) * step)
...
def comparison_stats(x: np.ndarray, y: np.ndarray) -> tuple[float, float, float]:
    mask = np.isfinite(x) & np.isfinite(y)
    if mask.sum() == 0:
    ...
    if x.size < 2 or np.nanstd(x) == 0.0 or np.nanstd(y) == 0.0:
        r2 = np.nan
    else:
...
def add_stats_box(ax, wrf_pair, aurora_pair) -> None:
    wrf_stats = comparison_stats(*wrf_pair)
    aurora_stats = comparison_stats(*aurora_pair)
    box = FancyBboxPatch(
        (0.035, 0.795),
        0.42,
        0.19,
    ...
    rows = [
        ("MB:", wrf_stats[0], aurora_stats[0], ".1f"),
        ("RMSE:", wrf_stats[1], aurora_stats[1], ".1f"),
        (r"$R^2$:", wrf_stats[2], aurora_stats[2], ".2f"),
    ]
    for (label, wrf_value, aurora_value, fmt), y_pos in zip(rows, [0.953, 0.887, 0.821], strict=True):
        ax.text(
            0.055,
            y_pos,
            label,
...
def draw_panel(ax, fig, spec: Species, panel: str, wrf_pair, aurora_pair, gridsize: int):
    ...
    limit = expanded_limit(axis_limit(spec, np.concatenate([all_x, all_y])))
    ...
    add_stats_box(ax, wrf_pair, aurora_pair)
...
def plot_figure(output: Path, wrf_station, aurora_station, obs_station, gridsize: int) -> None:
    ...
    handles = [
        Line2D([0], [0], color="#1f77b4", linestyle="-", linewidth=3.8, alpha=1.0, label="WRF-Chem"),
        Line2D([0], [0], color="#d62728", linestyle="-", linewidth=3.8, alpha=1.0, label="Aurora"),
    ]
    axes[0].legend(handles=handles, loc="upper right", frameon=False, handlelength=2.1, borderaxespad=0.2, fontsize=16)
